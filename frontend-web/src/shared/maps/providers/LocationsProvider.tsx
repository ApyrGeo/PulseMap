...
export const LocationsProvider = ({ children }: { children: ReactNode }) => {
  ...
  const confirmLocationEventHandler = useCallback(
    async (locationId: number) => {
      try {
        const updatedLocation = await confirmLocationEvent(locationId);
        setLocations((prev) =>
          prev.map((loc) => (loc.id === locationId ? updatedLocation : loc))
        );
      } catch (error) {
        console.error('Failed to confirm location event:', error);
        throw error;
      }
    },
    []
  ...
  return (
    <LocationsContext.Provider
      value={{
        ...
        likeLocation,
        unlikeLocation,
        confirmLocationEvent: confirmLocationEventHandler,
        rejectLocationEvent: rejectLocationEventHandler,
      }}
    >
      {children}
    </LocationsContext.Provider>
